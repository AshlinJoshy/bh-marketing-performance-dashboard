...
//
// Runs native SQL against the Metabase `leads` view via the dataset API with an
// API key. Set (missing → graceful "not connected"):
//   METABASE_URL      e.g. "https://metabase.yourco.com"
//   METABASE_API_KEY  a Metabase API key that can read the betterhomes DB
...
async function mbQuery(sql: string): Promise<any[][] | null> {
...
export async function getLeadsData(fromRaw: string, toRaw: string): Promise<LeadsData> {
  const connected = !!(process.env.METABASE_URL && process.env.METABASE_API_KEY);
